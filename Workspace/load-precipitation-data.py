...
Precipitation = Base.classes.Precipitation_Readings

def retrieveData(year, offset):
    precipitation_url_1 = "https://www.ncei.noaa.gov/cdo-web/api/v2/data?datasetid=GSOY&datatypeid=PRCP&startdate="
    precipitation_url_2 = "-01-01&enddate="
    precipitation_url_3 = "-12-31&limit=1000&offset="

    end_year = year + 9
    if year == 2022:
        end_year = year
    precipitation_url = precipitation_url_1 + str(year) + precipitation_url_2 + str(end_year) + precipitation_url_3 + str(offset)
    response = requests.get(precipitation_url, headers={
