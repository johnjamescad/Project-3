...
for (let i = 0; i < precipitationData.length; i++) {
  ...
  let val = precipitationData[i].Value
  sum += val;
   if (precipitationData[i].Year >= 1992 && precipitationData[i].Year < 2002){dec1.push(val)} 
    else if (precipitationData[i].Year >= 2002 && precipitationData[i].Year < 2012) {dec2.push(val)} 
    else if (precipitationData[i].Year >= 2012 && precipitationData[i].Year < 2022) {dec3.push(val)}}
 //console.log(dec3);
 var avg = sum / precipitationData.length;
